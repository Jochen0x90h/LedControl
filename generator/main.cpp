...
// print random sequence for tetris to std::cout only as template for hand tuned sequence
void printTetrisRandom() {
    auto &f = std::cout;
...
// generate lookup table for color temperature
void printColorTemperatureLookup() {
    //std::ofstream f(outputPath);
    //f << "#pragma once" << std::endl;
    //f << std::endl;
    auto &f = std::cout;

    static const int e12Table[12] = {10, 12, 15, 18, 22, 27, 33, 39, 47, 56, 68, 82};

    // lookup of hue and saturation for color temperature (E12: 24 - 43)
    f << "const Vector2<float> colorTemperature[] = {" << std::endl;
    int baseTemperature = 100;
    for (int i = 0; i < 20; ++i) {
        int e12Index = i % 12;
        int temperature = baseTemperature * e12Table[e12Index];
        //auto &ct = colorTemperature[index];

        // find color temperature entry
        for (auto &ct : colorTemperatures) {
            if (ct.step == 2 && ct.temperature == temperature) {
                //f << INDENT << '{' << int(ct.r) << ", " << int(ct.g) << ", " << int(ct.b) << "}, // " << temperature << "K" << std::endl;

                float3 rgb = {ct.r / 255.0f, ct.g / 255.0f, ct.b / 255.0f};
                float3 hsv = coco::math::rgb2hsv(rgb);

                f << INDENT << '{' << hsv.x << ", " << hsv.y << "}, // " << temperature << "K" << std::endl;
                //f << INDENT << '{' << int(hsv.x * 255.0f + 0.5f) << ", " << int(hsv.y * 255.0f + 0.5f) << "}, // " << temperature << "K" << std::endl;
            }
        }
...
int main(int argc, const char **argv) {
    ...
    // convert palettes (.palette files) in effects directory to C++ functions (lookup*.hpp)
    ...
    // print random function for Tetris effect (template for lookupTetrisRandom.hpp)
    printTetrisRandom();

    // generate lookup table for color temperature (lookupColorTemperature.hpp)
    printColorTemperatureLookup();//effectsDir / "lookupColorTemperature.hpp");
