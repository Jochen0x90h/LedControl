...
AwaitableCoroutine parametersMenu(Loop &loop, SSD130x &display, Input &buttons, EffectManager &effectManager, int presetIndex) {
	...
	while (true) {
		...
		co_await menu.show();
		co_await menu.wait(buttons);
	}
}

AwaitableCoroutine effectsMenu(Loop &loop, SSD130x &display, Input &buttons, EffectManager &effectManager) {
...
Coroutine mainMenu(Loop &loop, SSD130x &display, Input &input, EffectManager &effectManager) {
	// initialize and enable the display
	co_await drivers.resetDisplay();
	co_await display.init();
	co_await display.enable();
	...
	while (true) {
		...
		if (!showParameter) {
			...
			if (!redraw) {
				...
				// wait for input
				int s = co_await select(input.wait(seq), loop.sleep(parameterTimeout));
				if (s == 1) {
